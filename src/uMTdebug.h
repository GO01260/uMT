...
#define CHECK_TASK_MAGIC(task, message) { if (task->magic != uMT_TASK_MAGIC) Kernel.isr_Kn_FatalError(); }
#define CHECK_TIMER_MAGIC(timer, message) { if (timer->magic != uMT_TIMER_MAGIC) Kernel.isr_Kn_FatalError(); }
